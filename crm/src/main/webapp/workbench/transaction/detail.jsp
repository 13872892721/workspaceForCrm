...
<%@ page import="com.tanghaichao.crm.settings.domain.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="com.tanghaichao.crm.workbench.domain.Tran" %>
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//当前阶段可能性为0，说明前7个是黑圈，后面两个可能是红×，可能是黑×
			if("0".equals(currentPossibility)){
				for (int i=0;i<dvList.size();i++){
					DicValue dv = dvList.get(i);
					//如果是当前阶段，说明是红×
					if (currentStage.equals(dv.getValue())){
						//红×---------------------------------
						%>
						<span onclick="changeStage('<%=dv.getValue()%>','<%=i%>')" class="glyphicon glyphicon-remove mystage"
							  data-toggle="popover" data-placement="bottom"
							  data-content="<%=dv.getValue()%>" style="color: #FF0000;"></span>
						-----------
						<%
					//如果不是当前阶段，说明是黑×
					}else {
						...
						<span onclick="changeStage('<%=dv.getValue()%>','<%=i%>')" class="glyphicon glyphicon-remove mystage"
							  data-toggle="popover" data-placement="bottom"
							  data-content="<%=dv.getValue()%>" style="color: #000000;"></span>
						-----------
						<%
					}
				}

			//当前阶段可能性不为0，说明后面两个是黑×，前面7个可能是绿圈，可能是绿标，也可能是黑圈
			}else {
				int index = 0;
				for (int i=0;i<dvList.size();i++) {
					DicValue dv = dvList.get(i);
				...
				for (int i=0;i<dvList.size();i++) {
					...
					if (i>=point){
						...
						if (i==index){
							//绿标---------------------------------
						%>
						<spanv id="<%=i%>" onclick="changeStage('<%=dv.getValue()%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage"
							  data-toggle="popover" data-placement="bottom"
							  data-content="<%=dv.getValue()%>" style="color: #90F790;"></spanv>
						-----------
						<%
						...
						<span id="<%=i%>" onclick="changeStage('<%=dv.getValue()%>','<%=i%>')" class="glyphicon glyphicon-ok-circle mystage"
							  data-toggle="popover" data-placement="bottom"
							  data-content="<%=dv.getValue()%>" style="color: #90F790;"></span>
						-----------
						<%
